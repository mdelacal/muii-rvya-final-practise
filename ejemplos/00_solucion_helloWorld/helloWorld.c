...
static void draw( int pattern ) {
  ...
  GLfloat mat[4];        // El valor del material se asigna luego
  ...
  if (pattern == patt_id) {mat[0]=0; mat[1]=0; mat[2]=1;  mat[3]=1;}
  else {mat[0]=0; mat[1]=1; mat[2]=0;  mat[3]=1;}
...
static void init( void ) {
  ...
  if((patt_id_identic=arLoadPatt("data/identic.patt")) < 0) 
    print_error ("Error en carga de patron\n");
...
static void mainLoop(void) {
  ...
  for(j = 0, k = -1; j < marker_num; j++) {
    if((patt_id == marker_info[j].id) || (patt_id_identic == marker_info[j].id)) {
      if (k == -1) k = j;
  ...
  if(k != -1) {   // Si ha detectado el patron en algun sitio...
    ...
    draw(marker_info[k].id);   // Dibujamos los objetos de la escena
  }
